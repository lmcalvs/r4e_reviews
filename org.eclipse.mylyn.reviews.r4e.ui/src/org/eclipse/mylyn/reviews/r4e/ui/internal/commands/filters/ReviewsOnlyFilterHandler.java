...
public class ReviewsOnlyFilterHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (!oldValue) {
			R4EUIPlugin.Ftracer.traceInfo("Apply reviews only filter to ReviewNavigator");
			viewer.addFilter(filter);
		} else {
			R4EUIPlugin.Ftracer.traceInfo("Remove reviews only filter from ReviewNavigator");
