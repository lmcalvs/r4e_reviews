...
public class ScheduleMeetingInputDialog extends FormDialog {
	...
	protected void buttonPressed(int buttonId) {
        if (buttonId == IDialogConstants.OK_ID) {
        	...
        	if (null != validateResult) {
    			...
    			final ErrorDialog dialog = new ErrorDialog(null, R4EUIConstants.DIALOG_TITLE_ERROR, "No input given for Rule Set Version",
        				new Status(IStatus.ERROR, Activator.PLUGIN_ID, 0, validateResult, null), IStatus.ERROR);
    			dialog.open();
    			this.getShell().setCursor(this.getShell().getDisplay().getSystemCursor(SWT.CURSOR_ARROW));
    			return;
        	}
        	fDuration = Integer.getInteger(fDurationInputTextField.getText());
	...
	protected void createFormContent(final IManagedForm mform) {
		...
		//Meeting Start Time
	    final Label meetingStartTimeLabel = toolkit.createLabel(meetingComposite, R4EUIConstants.START_TIME_LABEL);
		...
		//Meeting Duration
	    final Label meetingEndTimeLabel = toolkit.createLabel(meetingComposite, R4EUIConstants.DURATION_LABEL);
		...
		//Meeting Location
	    final Label meetingLocationLabel = toolkit.createLabel(meetingComposite, R4EUIConstants.LOCATION_LABEL);
