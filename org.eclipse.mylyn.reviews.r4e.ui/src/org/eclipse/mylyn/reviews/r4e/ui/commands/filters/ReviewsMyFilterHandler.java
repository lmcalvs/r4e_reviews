...
public class ReviewsMyFilterHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
	    ...
	    final ReviewParticipantFilter filter = 
	    	((ReviewNavigatorActionGroup) R4EUIModelController.getNavigatorView().getActionSet()).getReviewsMyFilter();
		...
		if (!oldValue) {
			Activator.Ftracer.traceInfo("Apply my reviews filter to ReviewNavigator");
			viewer.addFilter(filter);
		} else {
			Activator.Ftracer.traceInfo("Remove my reviews filter from ReviewNavigator");
