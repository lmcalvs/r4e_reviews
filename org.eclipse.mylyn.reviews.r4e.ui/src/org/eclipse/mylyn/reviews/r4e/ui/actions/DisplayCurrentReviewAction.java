 ...
 */
public class DisplayCurrentReviewAction extends Action {
	 ...
	 */
	public DisplayCurrentReviewAction(ReviewNavigatorView aView, ViewerFilter aFilter, String aFilterName) {
	...
	public void run() {
		...
		IR4EUIModelElement rootElement = null;
		if (!fFilterApplied) {
			rootElement = R4EUIModelController.getActiveReview().getParent();
			Activator.Tracer.traceInfo("Change source input to root element " + rootElement.getName());
			fView.getTreeViewer().setInput(rootElement);
			fView.getTreeViewer().addFilter(fFilter);
			this.setChecked(true);
			fFilterApplied = true;
		} else {
			rootElement = R4EUIModelController.getRootElement();
			Activator.Tracer.traceInfo("Change source input to root element " + rootElement.getName());
			fView.getTreeViewer().setInput(rootElement);
