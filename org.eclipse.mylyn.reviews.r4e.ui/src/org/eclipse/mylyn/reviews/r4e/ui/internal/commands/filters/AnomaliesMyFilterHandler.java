...
public class AnomaliesMyFilterHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (!oldValue) {
			Activator.Ftracer.traceInfo("Apply current review only filter to ReviewNavigator");
			viewer.addFilter(filter);
		} else {
			Activator.Ftracer.traceInfo("Remove current review only filter from ReviewNavigator");
