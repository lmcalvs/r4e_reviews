...
public class AnomaliesMyFilterHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (!oldValue) {
			R4EUIPlugin.Ftracer.traceInfo("Apply current review only filter to ReviewNavigator");
			viewer.addFilter(filter);
		} else {
			R4EUIPlugin.Ftracer.traceInfo("Remove current review only filter from ReviewNavigator");
