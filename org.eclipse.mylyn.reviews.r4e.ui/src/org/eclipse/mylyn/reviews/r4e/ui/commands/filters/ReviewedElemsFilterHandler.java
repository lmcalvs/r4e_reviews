 ...
 */
public class ReviewedElemsFilterHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
	    ...
	    final ReviewedElemsFilter filter = 
	    	((ReviewNavigatorActionGroup) R4EUIModelController.getNavigatorView().getActionSet()).getReviewedElemsFilter();
		
	    final Object[] elements =  viewer.getExpandedElements();
	    boolean oldValue = HandlerUtil.toggleCommandState(event.getCommand());
	    
		if (!oldValue) {
			Activator.Ftracer.traceInfo("Apply hide reviewed elements filter to ReviewNavigator");
			viewer.addFilter(filter);
		} else {
			Activator.Ftracer.traceInfo("Remove hide reviewed elements filter from ReviewNavigator");
