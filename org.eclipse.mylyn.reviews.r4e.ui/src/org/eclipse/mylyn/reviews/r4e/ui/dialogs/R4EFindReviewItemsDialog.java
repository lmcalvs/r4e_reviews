	...
	protected void buttonPressed(int buttonId) {
        if (buttonId == IDialogConstants.OK_ID) {
			...
			try {
				...
				{
					//No files are found
					Activator.Tracer.traceWarning("No files found for this review item");
					...
					final R4EUIReviewItem uiReviewItem = new R4EUIReviewItem(uiReview, reviewItem, 
							R4EUIConstants.REVIEW_ITEM_TYPE_COMMIT, fReviewItemDescriptor);
					...
					for (int i = 0; i < tmpFilesSize; i++) {
						...
						if (null != tmpBaseVersion) {
							...
							baseVersion.setVersionID(tmpBaseVersion.getVersionID());
							R4EUIModelController.FResourceUpdater.checkIn(bookNum);
						}
						...
						targetVersion.setVersionID(tmpTargetVersion.getVersionID());
						R4EUIModelController.FResourceUpdater.checkIn(bookNum);
				...
				Activator.Tracer.traceError("Exception: " + e.toString() + " (" + e.getMessage() + ")");
				Activator.getDefault().logError("Exception: " + e.toString(), e);
				final ErrorDialog dialog = new ErrorDialog(null, "Error", "File not found error detected while adding review item ",
	    				new Status(IStatus.ERROR, Activator.PLUGIN_ID, 0, e.getMessage(), e), IStatus.ERROR);
				dialog.open();
